...
- Коли env-параметр додається, перейменовується або вилучається, узгоджено оновлюй `winhub.env.example`, runtime-код, update/install і distribution tests. Updater має переносити значення перейменованого параметра та видаляти лише явно перелічені deprecated keys після backup; не можна очищати всі невідомі env-змінні, бо вони можуть бути deployment-specific.
