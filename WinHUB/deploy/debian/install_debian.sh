...
INSTALL_PENDING_FILE="${ENV_DIR}/.installation-pending"
DATA_DIR="/var/lib/winhub"
LOG_DIR="/var/log/winhub"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
# shellcheck source=sync_server_files.sh
source "${SRC_DIR}/deploy/debian/sync_server_files.sh"
RECOVERY_WORK_DIR=""
RECOVERY_ARCHIVE=""
...
SRC_DIR="$(winhub_server_source "${SRC_DIR}")" || fail "Use a complete server checkout or release archive."
command -v apt-get >/dev/null && command -v systemctl >/dev/null || fail "This installer requires Debian with systemd."

if [[ ! -f "${ENV_FILE}" || -f "${INSTALL_PENDING_FILE}" ]]; then
  FRESH_INSTALL=true
fi
if [[ "${FRESH_INSTALL}" == true || -f "${DATA_DIR}/admin_recovery.txt" ]]; then
  [[ -t 0 ]] || fail "Initial setup and recovery require an interactive SSH or local terminal."
fi
...
if [[ "${FRESH_INSTALL}" == true ]]; then
  configure_public_host
  pause_for_operator "WinHUB will be configured for https://${PUBLIC_HOST}. Generated secrets will be preserved if setup needs to be resumed."
  mkdir -p "${ENV_DIR}"
  touch "${INSTALL_PENDING_FILE}"
fi
...
if [[ "$(readlink -f "${SRC_DIR}")" != "$(readlink -f "${APP_DIR}")" ]]; then
  info "Copying server components to ${APP_DIR}"
  winhub_sync_server_files "${SRC_DIR}" "${APP_DIR}"
else
  info "Server source is already located at ${APP_DIR}"
fi
...
if [[ "${FRESH_INSTALL}" == true ]]; then
  ...
  info "Session, database, enrollment, task-signing and history-search secrets are ready for the one-time recovery bundle."
  create_postgres_database
else
