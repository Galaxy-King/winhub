...
class OpenWebUIClient:
    ...
    def _request(self, method, path, **kwargs):
        ...
        deadline = time.monotonic() + Config.AI_REQUEST_TIMEOUT_SECONDS
            ...
            try:
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if time.monotonic() > deadline:
                        raise ValueError("Open WebUI response deadline exceeded")
                    raw.extend(chunk)
                    if len(raw) > Config.AI_MAX_HTTP_RESPONSE_BYTES:
                        raise ValueError("Open WebUI HTTP response exceeds the configured limit")
                try:
    ...
    def chat_completion(self, messages, model=None, max_output_bytes=None):
        payload = self._request("POST", "/api/chat/completions", json={
            "model": model or self.model,
            "messages": messages,
            "stream": False,
            # WinHUB only consumes text. Explicitly opt out of Open WebUI's
            # server-side tool resolution; no terminal/chat/session is attached.
            "tools": [],
            "tool_ids": [],
            "features": {"web_search": False, "code_interpreter": False,
                         "image_generation": False, "memory": False},
        })
        try:
            message = payload["choices"][0]["message"]
            if message.get("tool_calls") or message.get("function_call"):
                raise ValueError("Tool calls are not accepted from the AI provider")
        ...
        if len(encoded) > (max_output_bytes or Config.AI_MAX_OUTPUT_BYTES):
