...
def process_ai_templates_job():
    ...
    with global_app.app_context():
        try:
            ...
            db.session.rollback()
            log.exception("AI template queue failed")
        finally:
            db.session.remove()
...
def reload_scheduler_jobs(ignored_app=None):
    ...
    scheduler.add_job(func=process_ai_templates_job, trigger="interval", seconds=5,
                      id="ai_templates", max_instances=1, coalesce=True, replace_existing=True)
    scheduler.add_job(
