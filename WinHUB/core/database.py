...
class NewsletterCampaign(db.Model):
    ...
    result_report_json = db.Column(EncryptedText, nullable=True)
    result_report_status = db.Column(db.String(20), default="NotRequested", nullable=False)
    result_report_use_gpg = db.Column(db.Boolean, default=True, nullable=False)
    result_report_updated_at = db.Column(db.DateTime, nullable=True)
...
class AiTemplateDraft(db.Model):
    """Private immutable generation attempts; never executable tasks."""
    __tablename__ = 'ai_template_drafts'
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    actor_user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    prompt = db.Column(EncryptedText, nullable=False)
    source_code = db.Column(EncryptedText, nullable=True)
    language = db.Column(db.String(20), nullable=False)
    include_report = db.Column(db.Boolean, nullable=False, default=False)
    model = db.Column(db.String(150), nullable=False)
    status = db.Column(db.String(20), nullable=False, default='Queued', index=True)
    result_json = db.Column(EncryptedText, nullable=True)
    validation_json = db.Column(EncryptedText, nullable=True)
    error = db.Column(db.String(300), nullable=True)
    saved_template_ids = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False, index=True)
    started_at = db.Column(db.DateTime, nullable=True)
