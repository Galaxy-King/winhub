...
| `Single-level source folders` | По одному локальному шляху в рядку. Архівуються файли безпосередньо в цій папці, але вміст підпапок не обходиться (`Rar -r-` із маскою `\*`). Якщо потрібен увесь вміст дерева, використовуйте `Recursive source folders`. |
